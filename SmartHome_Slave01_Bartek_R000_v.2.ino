...
//uint8_t slave1_address[] = {0x94,0xA9,0x90,0x92,0xF6,0xD0};
...
//
  ...
  dataonboard.mode[0] = 1; dataonboard.mode[1] = 0; dataonboard.mode[2] = 0; dataonboard.mode[3] = 0;   
...
void loop() {
  ...
  digitalWrite(switchPin[0], dataonboard.state[0]);
  ...
  for(int channel=0; channel<4; channel++){
    if(dataonboard.mode[channel] != 0){
      ...
      else if(dataonboard.state[channel]==LOW){
        dataonboard.power[channel]=0;
      }
    }
  }
